...
三部分会合并去重后一起进入原有动量排名。固定池标的不会被修改、覆盖或删除；每天变化的只有两组动态成交金额 ETF。如果动态 ETF 已经存在于固定池中，只保留固定池中的那一份，不重复计算，也不会额外向后补足“新增 ETF 数量”。
...
这些固定池标的会和两组动态成交金额 ETF 一起参与动量排名，不等同于最终防御资产。策略最终防御资产单独设置为 `511880.XSHG`。
...
| `g.dynamic_money_etf_cache` | `{'date': None, 'yesterday': [], 'avg_5d': []}` | 当日动态 ETF 池缓存。 |
...
动态成交金额 ETF 池也按交易日缓存。策略用 `context.previous_date` 作为成交金额统计截止日，所以昨日成交金额和过去 5 日平均成交金额都不包含当日盘中数据。
...
```text
固定 ETF 池 + 昨日成交金额前5 + 过去5日平均成交金额前5
```
...
固定池和两组动态成交金额 ETF 合并去重后，每只 ETF 必须依次通过以下过滤条件，才会进入最终排名。
...
- 多资产轮动，不局限于 A 股；
- 固定池之外，每天自动纳入昨日成交金额和近 5 日平均成交金额最高的活跃 ETF；
