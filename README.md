...
3. 全市场 ETF 中过去 5 个交易日平均成交金额最大的 5 只。
4. 全市场 ETF 中过去 5 个交易日涨幅最大的 5 只。

四部分会合并去重后一起进入原有动量排名。固定池标的不会被修改、覆盖或删除；每天变化的只有三组动态 ETF。如果动态 ETF 已经存在于固定池中，只保留固定池中的那一份，不重复计算，也不会额外向后补足“新增 ETF 数量”。
...
这些固定池标的会和三组动态 ETF 一起参与动量排名，不等同于最终防御资产。策略最终防御资产单独设置为 `511880.XSHG`。
...
| `g.dynamic_avg_money_etf_count` | `5` | 取过去 5 日平均成交金额最大的 5 只全市场 ETF。 |
| `g.dynamic_avg_money_days` | `5` | 平均成交金额的计算窗口。 |
| `g.dynamic_return_etf_count` | `5` | 取过去 5 日涨幅最大的 5 只全市场 ETF。 |
| `g.dynamic_return_days` | `5` | 涨幅计算窗口。 |
| `g.dynamic_money_etf_cache` | `{'date': None, 'yesterday': [], 'avg_5d': [], 'return_5d': []}` | 当日动态 ETF 池缓存。 |
...
同样从全市场 ETF 中选择，取截至 `context.previous_date` 的最近 5 个交易日 `money` 数据，对每只 ETF 求平均成交金额：
...
只有最近 5 个交易日都有有效成交金额的 ETF 才参与排序。然后按 5 日平均成交金额从高到低取前 5。

### 过去 5 日涨幅前 5

同样从全市场 ETF 中选择，取截至 `context.previous_date` 的最近 5 个交易日 `close` 数据，计算区间涨幅：
...
只有最近 5 个交易日都有有效收盘价且收盘价大于 0 的 ETF 才参与排序。然后按 5 日涨幅从高到低取前 5。
...
- 过去 5 日平均成交金额前 5 只补充前两部分中都没有的 ETF；
- 过去 5 日涨幅前 5 只补充前三部分中都没有的 ETF；
- 如果三组动态 ETF 互相重复，只保留一份；
- 最多新增 15 只动态 ETF，但实际新增数量可能因为重复而少于 15 只。

## 候选 ETF 过滤流程

核心函数：`calculate_momentum_metrics_172(context, etf)`

固定池和三组动态 ETF 合并去重后，每只 ETF 必须依次通过以下过滤条件，才会进入最终排名。
...
- 多资产轮动，不局限于 A 股；
- 固定池之外，每天自动纳入昨日成交金额、近 5 日平均成交金额、近 5 日涨幅最高的活跃 ETF；
